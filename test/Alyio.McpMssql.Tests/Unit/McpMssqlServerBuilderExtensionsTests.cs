...
public sealed class McpMssqlServerBuilderExtensionsTests
{
    ...
    [Fact]
    public void WithMcpMssqlTools_Advertises_Write_Tool_When_The_Default_Profile_Allows_Write()
    {
        List<string> tools = RegisteredToolNames(
            ("MCPMSSQL_CONNECTION_STRING", "Server=.;Database=DefaultDb;"),
            ("MCPMSSQL_ALLOW_WRITE", "true"));

        Assert.Contains(WriteTools.ToolName, tools);
    }

    [Fact]
    public void WithMcpMssqlTools_Advertises_Write_Tool_When_Any_Named_Profile_Allows_Write()
    {
        // The gate is server-wide, not default-profile-only: one write-enabled
        // profile keeps the tool visible, and CommandService rejects the rest.
        List<string> tools = RegisteredToolNames(
